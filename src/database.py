...
def update_source_page_scraped_at(page_id: str, scraped_at: str):
    db = get_db()
    db.table("source_pages").update({"last_scraped_at": scraped_at}).eq(
        "id", page_id
    ).execute()
...
def is_post_exists(fb_post_id: str) -> bool:
    ...
    res2 = db.table("posts").select("id").eq("fb_post_id", fb_post_id).execute()
...
def create_scheduled_post(
    post_id: str, destination_page_id: str, scheduled_at: str
) -> str:
    db = get_db()
    res = (
...
def update_dest_last_scheduled(destination_page_id: str, scheduled_at: str):
    ...
    db = get_db()
    db.table("destination_pages").update({"last_scheduled_at": scheduled_at}).eq(
        "id", destination_page_id
    ).execute()


def mark_scheduled_post_done(scheduled_post_id: str):
    db = get_db()
    db.table("scheduled_posts").update({"status": "posted"}).eq(
...
def delete_scheduled_post(scheduled_post_id: str):
    db = get_db()
    db.table("scheduled_posts").delete().eq("id", scheduled_post_id).execute()


# ── Dedup ──────────────────────────────────────────────────


def save_dedup(fb_post_id: str, source_page_id: str, destination_page_id: str):

...
if page == "📊 Tổng quan":
    ...
    # Số đích
    dests = db.table("destination_pages").select("id").eq("is_active", True).execute().data
    col3.metric("Trang đích", len(dests))

    # Queue pending
    queue = db.table("scheduled_posts").select("id").eq("status", "pending").execute().data
    ...
    with col_a:
        ...
        upcoming = db.table("scheduled_posts") \
            .select("scheduled_at, destination_pages(fb_page_name), posts(content)") \
            .eq("status", "pending") \
            .order("scheduled_at").limit(10).execute().data
        ...
        if upcoming:
            ...
            for item in upcoming:
                rows.append({
                    ...
                    "Trang đích": (item.get("destination_pages") or {}).get("fb_page_name", "?"),
                    "Nội dung": ((item.get("posts") or {}).get("content") or "")[:80] + "...",
                })
        ...
        if logs:
            ...
            for log in logs:
                rows.append({
                    ...
                    "Kết quả": "✓ Thành công" if log["result"] == "success" else "✗ Thất bại",
...
# ══════════════════════════════════════════════════════════
    ...
    if groups:
        for g in groups:
            src_count  = len(db.table("source_pages").select("id").eq("group_id", g["id"]).execute().data)
            dest_count = len(db.table("destination_pages").select("id").eq("group_id", g["id"]).execute().data)
            pending    = len(db.table("scheduled_posts")
            ...
            with st.expander(
                ...
                if g["is_active"]:
                    if c1.button("🔴 Tắt nhóm", key=f"deact_{g['id']}"):
                        db.table("page_groups").update({"is_active": False}).eq("id", g["id"]).execute()
                        st.rerun()
                else:
                    if c1.button("🟢 Bật nhóm", key=f"act_{g['id']}"):
                        db.table("page_groups").update({"is_active": True}).eq("id", g["id"]).execute()
                        st.rerun()
    else:
...
# ══════════════════════════════════════════════════════════
    ...
    if pages:
        ...
        for p in pages:
            rows.append({
                ...
                "Lần cuối scrape": (p.get("last_scraped_at") or "Chưa")[:16].replace("T", " "),
                "ID": p["id"][:8] + "...",
            })
        st.dataframe(rows, use_container_width=True, hide_index=True)
        ...
        sel_page = next(p for p in pages if (p.get("fb_page_name") or p["fb_page_url"]) == sel_page_name)

        c1, c2 = st.columns(2)
        if sel_page["is_active"]:
            if c1.button("🔴 Tắt trang này"):
                db.table("source_pages").update({"is_active": False}).eq("id", sel_page["id"]).execute()
                st.rerun()
        else:
            if c1.button("🟢 Bật trang này"):
                db.table("source_pages").update({"is_active": True}).eq("id", sel_page["id"]).execute()
                st.rerun()
    ...
    with st.form("add_source"):
        ...
        page_name  = st.text_input("Tên trang (ghi chú)", placeholder="VD: Trang nấu ăn ABC")
        page_url   = st.text_input(
            "URL Facebook page",
            placeholder="https://www.facebook.com/pagename",
        )
        submitted = st.form_submit_button("✅ Thêm trang nguồn", use_container_width=True)
        if submitted:
...
# ══════════════════════════════════════════════════════════
    ...
    pages = db.table("destination_pages") \
    ...
    if pages:
        ...
        sel = next(p for p in pages if (p.get("fb_page_name") or p["fb_page_id"]) == sel_name)

        c1, c2, c3 = st.columns(3)
        if sel["is_active"]:
            if c1.button("🔴 Tắt trang"):
                db.table("destination_pages").update({"is_active": False}).eq("id", sel["id"]).execute()
                st.rerun()
        else:
            if c1.button("🟢 Bật trang"):
                db.table("destination_pages").update({"is_active": True}).eq("id", sel["id"]).execute()
                st.rerun()
        ...
        with st.expander("🔄 Cập nhật Access Token cho trang này"):
            ...
            if st.button("💾 Lưu token mới"):
                if new_token.strip():
                    db.table("destination_pages").update({"fb_access_token": new_token.strip()}) \
    ...
    with st.form("add_dest"):
        group_name = st.selectbox("Thuộc nhóm", list(group_map.keys()), key="dest_group")
        page_name  = st.text_input("Tên trang (ghi chú)", placeholder="VD: Trang Nấu Ăn Của Tôi")
        page_id    = st.text_input("Facebook Page ID", placeholder="123456789")
        token      = st.text_input("Page Access Token", type="password")
        submitted  = st.form_submit_button("✅ Thêm trang đích", use_container_width=True)
        if submitted:
...
# ══════════════════════════════════════════════════════════
    ...
    keys = db.table("apify_keys") \
        .select("*").order("usage_count").execute().data
            ...
            with st.expander(
                ...
                if k["is_active"]:
                    if col1.button("🔴 Tắt key", key=f"off_{k['id']}"):
                        db.table("apify_keys").update({"is_active": False}).eq("id", k["id"]).execute()
                        st.rerun()
                else:
                    if col1.button("🟢 Bật key", key=f"on_{k['id']}"):
                        db.table("apify_keys").update({"is_active": True}).eq("id", k["id"]).execute()
                        st.rerun()
    ...
    with st.form("add_apify_key"):
        ...
        limit   = st.number_input("Giới hạn an toàn (mặc định 450)", value=450, min_value=100, max_value=500)
...
# ══════════════════════════════════════════════════════════
    ...
    with col2:
        ...
        status_filter = st.selectbox("Trạng thái", ["pending", "posted", "failed", "Tất cả"])

    query = db.table("scheduled_posts") \
        .select("scheduled_at, status, retry_count, destination_pages(fb_page_name), posts(content, fb_post_id, image_urls, video_url)")

    if status_filter != "Tất cả":
        query = query.eq("status", status_filter)

    items = query.order("scheduled_at", desc=(status_filter == "posted")) \
